...
times = []
for line in lines
    a = prod(filter!(i -> isa(tryparse(Int, i), Number), split(line, " ")))
    
    if occursin("Time", line)
        global times = parse(Int, prod(filter!(i -> isa(tryparse(Int, i), Number), split(line, " "))))
    elseif occursin("Distance", line)
        global distances = parse(Int, prod(filter!(i -> isa(tryparse(Int, i), Number), split(line, " "))))
    end
end

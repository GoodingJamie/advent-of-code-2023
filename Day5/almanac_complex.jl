...
function find_seeds(seed_string::String)

    digits = map(i -> parse(Int, i), filter(i -> i != "", split(seed_string, " ")))
    paired_digits = Iterators.partition(digits, 2)

    for (start, range) in paired_digits
        println(start + range)
    end
...
seeds = find_seeds(almanac["seeds"]...)
